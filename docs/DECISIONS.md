...
| D-025 | Canon Model is the design-time authority for world knowledge; World Bible is a generated projection, never the Canon itself; Canon reaches worlds only through deterministic bootstrap compilation; runtime never reads Canon; docs/worldbuilding/ remains the principles layer with a promotion path | accepted | docs/adr/0021-world-bible-canon-model.md; docs/WORLD_BIBLE_ARCHITECTURE.md; docs/canon/ |
| D-026 | Simulation Evaluation Framework: deterministic, commit-pinned reports (Scenario Quality, Simulation Health, Rule Dependency Graph) over the canonical core; normative metrics (pass rate, determinism, purity, presentation/truth honesty, rule coverage) block merge via `npm run eval:compare`; livingness/emergence/diversity/knowledge/idle/chain-depth are informational direction signals; scenarios and metrics extend by adding files/pure functions; the evaluator never changes game behaviour | accepted | docs/adr/0022-simulation-evaluation-framework.md |
| D-027 | Risk and Exposure Model: risk is the observer-visible measure of the player's exposure to consequential outcomes — contextual, created by player/environment/time, feeding consequences/situations/discovery/narrative; the current single global counter fed by grid movement is a v1 approximation; the risk→audacity→fire chain is consciously deferred as a future domain (not a defect); a general Exposure vertical slice is the planned path | accepted | docs/adr/0023-risk-and-exposure-model.md |
