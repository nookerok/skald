...
const INQUIRY_PATTERNS: readonly [InquiryQueryId, readonly RegExp[]][] = [
  ...
  ["character_identity", [
    ...
    /^как\s+я\s+(?:здесь|сюда|тут)\s+(?:оказался|попал)/iu,
  ]],
  ["known_place_knowledge", [
  ...
  ["available_routes", [
    ...
    /^куда\s+(?:лучше\s+|стоит\s+|мне\s+)?(?:пойти|идти|направиться|двигаться)/iu,
  ...
  ["who_is_nearby", [
    ...
    /^опиши\s+(?:всех\s+)?(?:людей|человека|присутствующих|окружающих)/iu,
  ]],
  ["observed_reaction", [
