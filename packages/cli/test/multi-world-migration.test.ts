...
describe("multi-world persistence migration", () => {
  it("opens and migrates a real v1 database inside the migration transaction", () => {
    ...
    const narrations = reopened.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='turn_narrations'").all() as { name: string }[];
    expect(narrations.length).toBe(1);
    const entrypoints = reopened.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='world_entrypoints'").all() as { name: string }[];
