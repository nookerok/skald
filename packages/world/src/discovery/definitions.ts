...
const HEAT_CHANGES_MATERIAL: DiscoveryDefinition = {
  ...
  render(stage: DiscoveryStage): { title: string; question: string; summary: string } {
    switch (stage) {
      case "trace":
        return { title: "Тепло меняет свойства материалов", question: "Что происходит с предметами, когда они нагреваются?", summary: "Разные материалы сохраняют тепло по-разному." };
      case "hypothesis":
        return { title: "Тепло меняет свойства материалов", question: "Что происходит с предметами, когда они нагреваются?", summary: "Тепло не просто ощущается — оно меняет состояние предметов." };
      case "discovered":
        return { title: "Тепло меняет свойства материалов", question: "Что происходит с предметами, когда они нагреваются?", summary: "Нагрев материала способен изменить его свойства и поведение." };
    }
  },
};
