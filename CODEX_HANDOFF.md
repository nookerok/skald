...
- Observer-scoped Pilot Region map and progressive journeys completed in the working tree (ADR-0030): monotonic ObserverSpatialKnowledge (rumored < glimpsed < observed < traversed), server-owned map DTO v3 with revealZones/availableDetails, withheld coordinates, detail coverage/unlock policy, and route geometry clipped to physically traversed prefixes. Journeys now advance one deterministic tick at a time, can be interrupted by voice, never progress during offline ticks, and reveal destination only on completion. Authoring matrix/compiler checks, replay tests, partial-route tests and living-region integration are wired at docs/worldbuilding/riverwatch-map-knowledge-matrix.yaml. Validation: npm run validate PASS (122 files, 1441 passed, 1 skipped). Changes remain uncommitted, unpushed and undeployed.
